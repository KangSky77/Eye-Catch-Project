...
async function runAIAnalysis(droppedFile) {
    ...
    try {
        ...
        if (twoEyes) {
            ...
            // '%'는 질병 확률로 읽힌다(외부 리뷰) — 특징 점수 x/100 표기로 통일
            state.aiResult = `${resultText} (${t.eye_left} ${lp}/100, ${t.eye_right} ${rp}/100)`;
        } else {
            state.aiResult = `${resultText} (${t.score_label || 'AI 특징 점수'} ${d.probability}/100)`;
        }
