...
────────────────────────────────────────────────────────────────────
2026-09-13 감은 눈 대응 — 이 게이트는 재학습하지 않았다
────────────────────────────────────────────────────────────────────
감긴 눈꺼풀도 '눈 영역'이라 이 게이트를 0.9 안팎으로 통과했다(학습에 쓰지 않은 Commons 감은 눈
크롭의 58.9%). 검수한 감은 눈 크롭을 음성 계열로 더해 재학습해 봤지만(임계 0.60 기준)
    감은 눈 통과 58.9% → 21.4%, 정상 눈 거부 4.7% → 6.3%, AI 생성 감은 눈은 여전히 통과
로 뜬 눈 손해가 커서 채택하지 않았다. 대신 질문을 나눠 얼굴 모드 눈 크롭에 '뜸 여부' 판정기를
따로 뒀다 — scripts/build_eye_open_gate.py, app/models/eye_open_gate.npz.
평가 스크립트(evaluate_eye_gate.py)에는 익상편 눈·감은 눈 열을 남겨 이 게이트의 한계를 계속 보이게 한다.
"""
