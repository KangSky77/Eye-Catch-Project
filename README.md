...
- **4단계 판정**(2026-09-02): **강한 혼탁 특징 감지**(≥50) / **혼탁 특징 감지**(25~50, 재촬영·검진 권장) /
  **사진만으로 판단 어려움**(2~25 — 눈 클로즈업 재촬영 권유) / **뚜렷한 진행성 혼탁 특징 없음**(<2).
  "정상"이라는 말은 쓰지 않습니다 — 모델은 눈에 보이는 진행성 혼탁 특징만 보며, 초기 백내장은 겉사진에
  나타나지 않을 수 있습니다(외부 테스트: AI 생성 얼굴의 옅은 초기 혼탁 5장이 0~4.9점). 문구도 "백내장 판별"이
  아니라 "혼탁 특징 분석"으로 범위를 좁혔습니다.
- **좌/우 눈 개별 분석** — 얼굴 사진에서 양쪽 눈을 따로 판정하고,
  한쪽만 위험하면 **"편측 의심" 배지** 표시 (편측 백내장 대응)
- MTCNN 얼굴→눈 크롭 (얼굴 사진/눈 클로즈업 모두 지원). 얼굴 사진은 편의 기능이고 **눈을 한쪽씩 가까이 찍는
  쪽이 훨씬 정확**합니다 — 촬영 팁과 결과 화면(애매한 신호일 때)에서 클로즈업 재촬영을 권합니다.
- **눈/비-눈 게이트** — 얼굴 사진의 눈 크롭과 눈 클로즈업 모두 "정말 눈이 보이는가"를 확인합니다. 눈을 감았거나
  선글라스·안대로 가린 얼굴도 MTCNN은 눈 좌표를 돌려주기 때문에, 예전에는 피부·검정 조각이 모델에 들어가
  '정상 0.0%'가 나갔습니다(실측). ResNet18 임베딩 위의 1층 분류기(`scripts/build_eye_gate.py`)로 교체해
  홀드아웃 눈 99.7% 통과 / 가려진 눈 0.003~0.016(거부) / 이마·볼·입·안저·풍경 거부.
...
- **반사 보류 규칙**(2026-09-02): 반사는 판정을 '위험' 쪽으로만 밀므로, 반사가 있어도 모델이 '뚜렷한 특징 없음'이면
  그대로 내보내고 위험·경계일 때만 보류합니다. 예전 무조건 보류는 데이터셋 백내장 사진의 3.0%(54/1,823)를 거부했습니다.
  반사 낀 진짜 백내장은 여전히 재촬영을 요청하지만(놓치는 게 아니라 플래시 끄고 다시), 정상 눈의 불필요한 재촬영은 사라집니다.
...
### 9️⃣ **실사용 리뷰 반영** (2026-08-29, master에서 머지)
- 🌐 **리포트가 언어를 따라옵니다** — 분석 결과·암슬러·문진 소견을 문자열이 아니라 언어 중립 원자료로 저장하고
  그릴 때 현재 언어로 만듭니다(`formatCataractResult` 등). 3줄 요약은 생성 언어로 고정되므로 언어를 바꾸면
  "현재 언어로 다시 생성" 버튼을 안내합니다.
- 📐 **암슬러 격자를 실물 크기(10cm, 5mm 칸 20×20)로** — 시력검사의 화면 보정(px/mm)을 재사용해 그리고, 그 크기가
  중심 20°를 덮는 거리를 계산해 안내합니다. 보정 전에는 근사치임을 밝힙니다.
- 🖼️ 결과 화면에 **분석한 사진**을 함께 보여줍니다. 문진 '네/아니오' 버튼의 시각 무게를 같게 해 응답 편향을 줄이고,
  이미 답이 정해진 문항은 건너뛰며(skipIf), 진행률을 위험요인 구간부터 끝까지 이어서 표시합니다.
- 🚪 눈 클로즈업 폴백 검증기(중심 벡터)의 임계값 0.55 → 0.62 — `scripts/probe_eye_gate.py`로 재현되는 실측 근거.
  (2026-09-02부터는 `eye_gate.npz` 분류기가 우선이고, 이 값은 파일이 없을 때의 폴백입니다.)
...
- 익상편 오분류는 데이터셋 내부 표본 60장 기준 **68.3% → 0%**, 밝은 홍채 오탐 5.0% → 1.7%, 백내장 검진
  안내율(위험+경계) 93.3% 유지(`scripts/eval_pterygium.py`). 다만 학습에 쓰지 않은 위키미디어 익상편 5장에서는
  3/5 → 2/5 오분류로, 캐글 데이터에 특화된 부분이 있어 **완전히 해결된 것은 아닙니다.**
- **민감도 하락은 실제 놓침이 아니라 라벨 오류였습니다(2026-09-02 확인).** v6이 test에서 놓친 백내장 7~9장을 전부
  열어 보니 홍채가 맑은 정상 눈(1장은 검은 조각 사진)으로, 백내장 클래스에 잘못 들어간 사진이었습니다. v5가 이들을
  100%로 잡은 것은 v5 학습 분할(train 10장·val 2장)에 있어 **외운** 결과였고, 분할이 바뀌며 test로 새어 나온 것입니다.
  10장을 `dataset_raw/label_errors_1_cataract/`로 옮기고(`data/label_errors.json`) 같은 분할에서 test의 해당 8장만
  제외해 재계산하면 **민감도 100%(FN 0) / 특이도 99.9%(FP 2) / AUC 1.000**(백내장 288·정상 2,230, TTA 없음)입니다.
  메타데이터 `label_corrections` 항목에 기록했습니다. 재학습 결과가 아니므로 다음 재학습 때 이 10장이 빠진 분할로
  지표가 다시 나옵니다.
...
### 4. 초기·옅은 혼탁은 사진으로 잡히지 않습니다
외부 테스트(2026-09-02, AI 생성 얼굴 5장 — 저작권 문제 없이 만든 합성 이미지)에서 매우 옅은~경도 혼탁은
0·0·0·0.1·4.9점으로 나와 모두 "특징 없음"으로 표시됐고, 과장된 진행성 혼탁만 76.5·100점이었습니다. 합성 이미지의
혼탁이 실제 수정체의 광학 특징을 재현한다는 보장은 없지만, 제품 관점의 신호는 분명합니다 — 이 앱은
**사진에서 보이는 진행성 혼탁 특징을 확인**하는 도구이지 초기 백내장을 감지하는 도구가 아닙니다(NEI도 초기 백내장은
증상이 없을 수 있고 진단은 산동 검사로 한다고 안내합니다). 그래서 낮은 점수를 "정상"으로 쓰지 않고 2~25점은
"사진만으로 판단 어려움"으로 분리해 눈 클로즈업 재촬영을 권합니다. 임상 라벨(안과 전문의·산동 검사)이 붙은 실제
사진으로 초기/중기/진행성별 민감도, 피부색·홍채색별, 기종·조명별, 얼굴 사진 대 클로즈업 정확도를 측정하기 전에는
그 이상을 주장할 수 없습니다.

### 5. 안저(망막) 사진을 거르지 못합니다
...
# 응답
{
  ...
  "closeup_suggested": false,    # uncertain, 또는 얼굴 모드의 borderline → 눈 클로즈업 재촬영 권유
  "sharpness": 0.0243            # blurry 판정 근거(선명도, 보류일 때만 의미 있음)
}
# invalid = 눈 사진 아님, eyes_hidden = 얼굴에서 눈이 감김/가려짐, hold = 플래시 반사(위험·경계일 때만),
# blurry = 흔들림/초점 — 전부 판정 대신 재촬영 안내. 검사 순서: 흔들림 → 눈 게이트 → 반사+모델
```
...
### 눈/비-눈 게이트 재학습
```bash
python scripts/build_eye_gate.py             # app/models/eye_gate.npz 생성 (+ 홀드아웃 성능 출력)
python scripts/build_eye_gate.py --dry-run   # 저장 없이 성능만
```
양성은 데이터셋 눈 3,600장, 음성은 눈 사진의 중앙을 덮은 '가려진 눈'·모서리 피부·단색·노이즈·풍경 크롭 1.2만 장.
게이트 파일이 없으면 앱은 예전 중심 벡터 유사도로 폴백하는데, 그 방식은 이마·볼·안저·풍경을 눈으로 통과시키므로
파일을 지우지 마세요.
...
| 얼굴 사진인데 "눈이 감겨 있거나 가려진 것 같아요" | 눈을 감았거나 안경·선글라스·안대로 가려 눈 게이트가 크롭을 거부 | 눈을 크게 뜨고 안경을 벗고 재촬영. 정상 눈인데 반복되면 `eye_gate.npz` 임계값(0.207) 재검토 |
| 옅은 혼탁이 있는 것 같은데 낮은 점수 | 모델은 뚜렷한 진행성 혼탁 특징에만 반응 → 2~25점은 "판단 어려움" | 눈을 한쪽씩 가까이 찍어 재확인. 반복되면 안과 검진 — 앱은 초기 백내장을 감지하지 못합니다 |
